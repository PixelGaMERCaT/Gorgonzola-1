package frc.robot;

/**
 * A class containing all the IDs for buttons on joysticks.
 */
public class ButtonMap {
    //Joysticks:
    public static final int LEFT_STICK = 0;
    public static final int RIGHT_STICK = 1;
    public static final int AUX_STICK = 5;
    public static final int DRIVE_SAFETY = 1;
    public static final int CAM_MANUAL = 3;
    public static final int KNIFE_DEPLOY = 2;
    public static final int CAM_AUTO = 2;
    public static final int GEAR_SHIFT = 1;
    
    //Operator:
    public static final int SHOULDER_STICK = 1;
    public static final int WRIST_STICK = 3;
    
    /*public static final int GROUND_PICKUP = 1;
    public static final int BALL_LOW = 2;
    public static final int BALL_MID = 3;
    public static final int BALL_HIGH = 4;
    public static final int BALL_INTAKE = 5;
    public static final int BALL_OUTPUT = 6;
    public static final int AUX_SAFETY = 7;
    public static final int SUCC_TOGGLE_BUTTON = 8;
    public static final int POS_MANUAL_OVERRIDE = 9;
    public static final int FULL_MANUAL_OVERRIDE = 10;
    //POV (represents an angle in degrees)
    public static final int HATCH_HIGH = 0;
    public static final int STOW = 90;
    public static final int HATCH_LOW = 180;
    public static final int HATCH_MID = 270;*/
    
    //Alternate controls map
    
    public static final int STOW = 1;
    public static final int HATCH_LOW = 2;
    public static final int HATCH_MID = 3;
    public static final int HATCH_HIGH = 4;
    public static final int HB_SWITCH = 8;
    public static final int GROUND_PICKUP = 1;
    public static final int BALL_LOW = 2;
    public static final int BALL_MID = 3;
    public static final int BALL_HIGH = 4;
    public static final int AUX_SAFETY = 5;
    public static final int SUCC_TOGGLE_BUTTON = 6;
    public static final int POS_MANUAL_OVERRIDE = 9;
    public static final int FULL_MANUAL_OVERRIDE = 10;
    //POV (represents an angle in degrees)
    public static final int BALL_INTAKE = 0;
    public static final int BALL_OUTPUT = 180;
    

}